...
type priorityWorker struct {
	...
	workers    []coroutine.Worker // 各个优先级，没有优先级都是抢占式工作机制
}

func NewPriorityWorker(number, size, priorities int) coroutine.Worker {
	...
	w := &priorityWorker{
		...
		size:       size,
		closed:     0,
		workers:    make([]coroutine.Worker, priorities),
	}
...
func (pw priorityWorker) Push(t coroutine.Task) bool {
	if pw.isClosed() {
		return false
	}
	if pt, ok := t.(coroutine.PriorityTask); ok {
		// 获取任务中的优先级
