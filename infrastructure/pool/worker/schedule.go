...
type priorityWorker struct {
	...
	workers    []worki.Worker // 各个优先级，没有优先级都是抢占式工作机制
}

func NewPriorityWorker(number, size, priorities int) worki.Worker {
	...
	w := &priorityWorker{
		...
		size:       size,
		closed:     0,
		workers:    make([]worki.Worker, priorities),
	}
...
func (pw priorityWorker) Push(t taski.Task) bool {
	if pw.isClosed() {
		return false
	}
	if pt, ok := t.(taski.PriorityTask); ok {
		// 获取任务中的优先级
