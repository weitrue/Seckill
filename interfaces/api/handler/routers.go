...
func InitRouters(g *gin.Engine) {
	...
	activityCBMW := middlewares.CircuitBreakMiddleware(activityCB)
	// 熔断器 不使用身份校验中间件
	activity := g.Group("/activity").Use(activityCBMW, middlewares.AuthMiddleware(false))
	...
	sub := g.Group("/activity/subscribe").Use(middlewares.AuthMiddleware(true))
	...
	mdws := []gin.HandlerFunc{
		middlewares.CircuitBreakMiddleware(shopCB),
