...
const (
	...
	isAdminKey      = "X-Is-Admin"
	userIdKey       = "X-User-Id"
	departmentIdKey = "X-Department-Id"
	companyIdKey    = "X-Company-Id"
	roleIdKey       = "X-Role-Id"
)
...
func (t *Token) Visit(fn func(key, val string) bool) {
	...
	fn(isAdminKey, convert.ToString(t.IsAdmin))
	fn(userIdKey, convert.ToString(t.UserId))
	fn(departmentIdKey, convert.ToString(t.DepartmentId))
	fn(companyIdKey, convert.ToString(t.CompanyId))
...
func FromMD(md metadata.MD) (*Token, bool) {
	...
	if ias := md.Get(isAdminKey); len(ias) > 0 {
		t.IsAdmin = convert.ToBool(ias[0])
	}
	if uis := md.Get(userIdKey); len(uis) > 0 {
		t.UserId = convert.ToInt64(uis[0])
	}
	if dis := md.Get(departmentIdKey); len(dis) > 0 {
		t.DepartmentId = convert.ToInt64(dis[0])
	}
	if cis := md.Get(companyIdKey); len(cis) > 0 {
		t.CompanyId = convert.ToInt64(cis[0])
	}
